...
Each token is then lowered and normalized by removing every character that is not a lowercase ASCII letter, using the regular expression \verb|[^a-z]+|.
...
\subsection{Initializing Clusters with K-Means++}
To give the algorithm a good starting point, it's necessary to pick initial centroids that have a good chance of lying in different clusters. The K-Means++ initialization strategy addresses this by selecting points that are far apart and representative of the data distribution:
\begin{enumerate}
    \item Pick the first centroid uniformly at random from the dataset.
    \item For each subsequent centroid (until $k$ centroids are chosen):
    \begin{enumerate}
        \item Compute the distance from each point to its nearest already-selected centroid.
        \item Select the next centroid with probability proportional to the squared distance.
    \end{enumerate}
\end{enumerate}
